...
import { legacyNetworkingStudyProject } from "../src/data/study/networking";
import {
  createInitialState,
  dateKey,
  dateInTimeZone,
...
function submission(learnerId = "A", packId = "ch1-01"): Progress {
...
function review(targetLearnerId = "A", packId = "ch1-01"): PeerReview {
...
function answerPack(state: StudyState, learnerId = "A", packId = "ch1-01") {
...
function populatedState(): StudyState {
  ...
  answerPack(state);
  state.sessions.push({ id: "A:session-1", projectId, packId: "ch1-01", chapterId: "1", learnerId: "A", date: "2026-09-21", minutes: 45, note: "完成前半包", createdAt: now });
  return state;
}

test("the current plan includes all photographed chapter-one exercises and the unchanged remaining chapters", () => {
  ...
  assert.equal(packs.filter(pack => pack.kind === "practice").length, 13);
  assert.equal(packs.filter(pack => pack.kind === "lab").length, 3);
  assert.ok(packs.every(pack => pack.base && !pack.archived));
  const first = networkingStudyProject.chapters[0];
  const questions = first.packs.flatMap(pack => pack.questions);
  const questionIds = questions.map(question => question.id);
  for (let number = 1; number <= 28; number++) assert.equal(questionIds.filter(id => id === `ch1-r${String(number).padStart(2, '0')}`).length, 1);
  for (let number = 1; number <= 34; number++) assert.equal(questionIds.filter(id => id === `ch1-p${String(number).padStart(2, '0')}`).length, 1);
  assert.equal(questions.filter(question => /^ch1-[rp]\d+$/.test(question.id)).length, 62);
  assert.ok(first.packs.filter(pack => pack.kind !== 'lab').flatMap(pack => pack.questions).every(question => /^ch1-[rp]\d+$/.test(question.id)), 'no old self-authored questions remain in the active chapter');
  assert.deepEqual(networkingStudyProject.chapters.slice(1), legacyNetworkingStudyProject.chapters.slice(1));
  assert.match(packs.find(pack => pack.id === "8-P")!.bookPractice, /p\.452：TLS实验、IPsec实验/);
  const state = createInitialState();
  assert.equal(validateStudyState(state), true);
  assert.deepEqual([state.progress, state.answers, state.sessions, state.reviews], [[], [], [], []]);
  assert.equal(getPackStatus(state, projectId, "ch1-01", "A"), "unrecorded");
...
test("initial states are independent and all record keys start with the acting learner", () => {
  ...
  assert.equal(progressKey(projectId, "ch1-01", "A"), "A:computer-networking:ch1-01");
});
...
test("completion requires a current submission, evidence, and a distinct actual peer", () => {
  ...
  state.progress.push(submission());
  answerPack(state);
  assert.equal(getPackStatus(state, projectId, "ch1-01", "A"), "submitted");
  state.reviews.push(review());
  assert.equal(getPackStatus(state, projectId, "ch1-01", "A"), "passed");
  ...
  assert.equal(getPackStatus(state, projectId, "ch1-01", "A"), "studying");
});
...
test("editing a submission invalidates its old review and the newest current review wins", () => {
  ...
  assert.equal(getPackStatus(state, projectId, "ch1-01", "A"), "revision");
...
test("statistics keep the base denominator stable and sum only the learner's actual sessions", () => {
  ...
  state.progress.push(submission("A", "1-X-01"), submission("A", "ch1-02"), submission("B"));
  answerPack(state, "A", "ch1-02");
  answerPack(state, "B");
  ...
  assert.deepEqual(getProjectStats(state, projectId, "A"), { completed: 2, verified: 1, submitted: 1, total: 68, minutes: 75, percent: 3 });
...
test("adding questions or changing answers invalidates approval until a fresh submission is reviewed", () => {
  ...
  assert.equal(getPackStatus(state, projectId, "ch1-01", "A"), "revision");
  state.answers.push({ id: `A:${projectId}:ch1-01:extra-q`, projectId, packId: "ch1-01", questionId: "extra-q", learnerId: "A", text: "补充作答", updatedAt: later });
  assert.equal(getPackStatus(state, projectId, "ch1-01", "A"), "revision");
  state.progress[0].updatedAt = later;
  assert.equal(getPackStatus(state, projectId, "ch1-01", "A"), "submitted");
  state.reviews[0].submissionUpdatedAt = later;
...
test('book enrollment scopes members independently, while former members keep historical attribution', () => {
  ...
  assert.equal(getReviewStatus(state, projectId, 'ch1-01', 'A'), 'passed');
  assert.ok(validateStudyState(state));
});
...
test('personal completion survives editing and peer revision; every current reviewer can block verification', () => {
  ...
  state.reviews.push({ ...review(), id: `C:${projectId}:ch1-01:A`, learnerId: 'C', outcome: 'changes' });
  assert.equal(getReviewStatus(state, projectId, 'ch1-01', 'A'), 'changes');
  assert.equal(getProjectStats(state, projectId, 'A').completed, 1);
  assert.equal(getProjectStats(state, projectId, 'A').verified, 0);
  state.progress[0].status = 'studying';
  assert.equal(getReviewStatus(state, projectId, 'ch1-01', 'A'), 'unsubmitted');
  assert.equal(isPackCompleted(state, projectId, 'ch1-01', 'A'), true);
