...
数据库保护已有章节、学习包和习题的 ID，不允许通过删除或移动已有结构破坏历史引用；可追加内容。单条 JSON 上限 1 MB，时长大于 0 且不超过 1500 分钟，以兼容夏令时回退的 25 小时日。计时原始片段须为合法、正向且不重叠的时间区间；手动调整日期或分钟可以保留原始片段，不要求二者相等。时长记录的更正与撤销也使用版本检查。首次完成日期保留。
