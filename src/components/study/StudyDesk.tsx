...
export default function StudyDesk({ state, actor, project, pack, chapter, viewLearner, onViewLearner, onSelectPack, save, notify, onRecord, onReview, onDeleteQuestion, busy, managementError }: {
  ...
  const canEdit = viewing === actor && actorIsMember && !pack.archived;
  ...
  return <div className={s.desk}>
    <div className={s.deskToolbar}><label>章节<select value={chapter.id} aria-label="选择学习章节" onChange={e => { const c = project.chapters.find(item => item.id === e.target.value); const next = c?.packs.find(p => !p.archived); if (next) onSelectPack(next); }}>{project.chapters.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}</select></label><label>学习包<select value={pack.id} aria-label="选择学习包" onChange={e => { const p = chapter.packs.find(p => p.id === e.target.value); if (p) onSelectPack(p); }}>{chapter.packs.filter(p => !p.archived || p.id === pack.id).map(p => <option key={p.id} value={p.id}>{p.archived ? '历史' : p.id.startsWith('extra-') ? '+' : p.id.replace(/^ch1-/, '1-')} · {p.title}</option>)}</select></label></div>
    <section className={s.lessonIntro}><div className={s.cardEyebrow}><BookOpen size={14} />{kindNames[pack.kind]}<span>{pack.base ? '基础学习包' : '追加学习包'}</span></div><h2>{pack.title}</h2><p>{pack.reading || '按照自己的学习材料推进，在下方记下产物和练习。'}</p><div className={s.lessonMeta}><span><Clock3 size={13} />预计 {pack.minutes ? `${pack.minutes} 分钟` : '自行安排'}</span><span><FileText size={13} />{pack.questions.length} 道练习</span></div>{pack.bookPractice && <details className={s.bookPractice}><summary>{pack.kind === 'lab' ? '实验步骤与参考资料' : '原书练习与批次说明'} <ChevronRight size={13} /></summary><p><StudyText text={pack.bookPractice} /></p></details>}</section>
    ...
    {!canEdit && <div className={s.readonly}><Eye size={14} />{pack.archived ? '此前学习记录 · 原答案、完成日期和学习时间已保留，请在新版学习包继续作答。' : `正在查看 ${learner?.name || '成员'} 的记录；仅项目中的本人可以作答和记录。`}</div>}
    ...
    {pack.id === 'ch1-R' && <section className={s.chapterReview}><div className={s.sectionTitle}><h2>第一章总复习</h2><span>原题与小节共用一份答案</span></div><p>按小节回看 R1–R28，再完成 P1–P34。点击对应批次继续作答，复习时可切换伙伴的记录进行核对。</p><div>{chapter.packs.filter(item => !item.archived && item.id !== pack.id && item.questions.some(question => /^ch1-[rp]\d+$/.test(question.id))).map(item => {
    ...
    {currentQuestions.length > 4 && <nav className={s.questionNav} aria-label="跳转到练习">{currentQuestions.map(question => {
      ...
      return <button key={question.id} type="button" data-answered={done} aria-label={`${label}${done ? '，已作答' : '，待作答'}`} onClick={() => { const target = document.getElementById(`study-question-${question.id}`); target?.focus({ preventScroll: true }); target?.scrollIntoView({ block: 'start', behavior: 'smooth' }); }}>{done && <Check size={11} />}{label}</button>;
    })}</nav>}
    ...
    {!pack.archived && viewing !== actor && actorIsMember && reviewStatus === 'pending' && <button className={s.primaryButton} onClick={() => onReview(viewing)}><CheckCheck size={15} />为伙伴验收这份学习</button>}
  </div>;
}
