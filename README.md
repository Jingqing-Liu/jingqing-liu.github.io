...
- **习题与笔记**：答案与笔记自动保存、按题号跳转、添加实际原书题、学习完成与互检分别记录。作答支持文字颜色、自选颜色、荧光标记、字号、标题、上下标、列表、引用、行内代码、代码块、链接、表格及撤销／重做。
- **公式作答**：选择分数、根号、求和、积分、矩阵等模板，或输入 LaTeX，实时预览并选择行内／独立公式；点击已有公式可再次编辑。公式、颜色和排版随作答自动保存，同书成员查看该成员的作答时可看到完整效果。
...
富文本作答沿用现有答案字段与同步队列，不需要新建表或运行 SQL。新答案以 `study-rich-v1:` 标识的受限文档格式保存；旧纯文本／Markdown 答案可以继续查看、编辑，单纯查看不会改写旧数据。页面的单题保存容量为 20,000 个字符（包含排版结构），超限会保留当前草稿并提示精简。共享空间仍是每人独立作答、成员相互查看，自动同步遵循现有云端保存和刷新机制。更新后，各设备刷新页面即可使用新版编辑器。
